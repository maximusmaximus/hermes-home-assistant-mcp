...
#### 4. 🖥️ System Actions & Host Controls
- **Underlying Capabilities:** Home Assistant Core restart, Host reboot/shutdown, YAML reload, Agent restart, System health status.
- **Card:** Displays Home Assistant operational state (`RUNNING`, version), MCP server responsiveness, and agent status.
- **Inline Action Buttons:**
  - `[ 🔄 Restart HA Core ] [ 🔌 Reboot HA Host ]`
  - `[ 🛑 Shut Down Host ] [ ⚙️ Reload All YAML ]`
  - `[ 🤖 Restart Agent ] [ 🏥 System Health ]`
- **Safety Confirmation Rule:** Destructive actions (Reboot, Shutdown, Core Restart) MUST invoke confirmation dialogs (`choices: ["✅ Confirm", "❌ Cancel"]`) before firing.
